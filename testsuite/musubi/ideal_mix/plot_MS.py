...
#  os.remove(dbname)
  ...
  sqlcon = gleaner.tracking_to_db(fname = 'tracking_MS/*MS_line*.res', \
                                  dbname=dbname, tabname='MS')
## -------------------------------------------------------------------------- ##
print ('Concentration over height:')
fig = mplt.figure()
ax = fig.add_subplot(111)
moleDens0 = 100
pot = 50e-3
###Concentration
na_conc = analy_conc(x1, moleDens0, length, -pot, pot, 1.0)
mplt.plot(x1*1e9, na_conc, 'x', color = 'r', label = '$Na^+$, Analytic')
cl_conc = analy_conc(x1, moleDens0, length, -pot, pot, -1.0)
mplt.plot(x1*1e9, cl_conc, 'o', color = 'r', label = '$Cl^-$, Analytic')
# Simulation result
get_data_for_cols = ['coordY','Na_mole_density_phy','Cl_mole_density_phy']
[x, y, z] = gleaner.get_columns(sqlcon, tabname='MS', \
                             columns=get_data_for_cols)
x, y, z = zip(*sorted(zip(x,y,z))) # sort of needed
xnew = convertToNumpy(x,1e9)
ynew = convertToNumpy(y,1.0)
znew = convertToNumpy(z,1.0)
mplt.plot(xnew, ynew, '-', color='k', label = '$Na^+$, Simulation')
mplt.plot(xnew, znew, '--', color='k', label = '$Cl^-$, Simulation')
...
mplt.savefig('ConcentrationOverHeight.pdf', dpi=100, format='pdf', \
